...
import { PoDataSave, PoDataSaveRequest, PoExcelResponse, PoGetRequest, PoResponse, PoSaveResponse } from "../protos/generated/po/po_pb";
...
export class PoGrpcService{
    ...
    public GetExcel(isArchive : boolean): Promise<string>{
        const req = new PoGetRequest();
        req.setIsarchive(isArchive);
        return new Promise<string>((resolve,reject)=>{            
            grpc.unary(PoSrv.GetExcel,{
                request: req,
                host: Constant.ServiceHost,
                metadata: ServiceHelper.CreateAuthToken(this.account),
                onEnd: res =>{                    
                    const {status, message} = res;
                    if (status === grpc.Code.OK && message) {                        
                        let result = message.toObject() as PoExcelResponse.AsObject;
                        if (result != null)
