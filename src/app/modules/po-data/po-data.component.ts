...
export class PoDataComponent implements OnInit {
  ...
  public IsExportingExcel: number = 0;
  public ExcelUrl = "";
  OnExportToExcelClick()
  {
    ...
    if (isArchive != null)
    { 
      this.IsExportingExcel = 1;
      service.GetExcel(isArchive).then(data => {
        this.ExcelUrl = "";
        if (!data || data == "") {
            alert("There is no excel to download");
            this.IsExportingExcel = 0;
        }
        else {
            this.ExcelUrl = Constant.PoDataExcelAddress + data;
            this.IsExportingExcel = 2;
        }
    }).catch(x=>{
